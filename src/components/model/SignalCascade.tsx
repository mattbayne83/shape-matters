...
export function SignalCascade({ levels, fidelityRate, semantic, compact }: SignalCascadeProps) {
  ...
  return (
    <svg viewBox="0 0 300 220" className="w-full h-auto" role="img" aria-label={`Signal cascade showing ${data[data.length - 1]?.f.toFixed(1) ?? 0}% fidelity remaining after ${levels} layers at ${fidelityRate}% per-layer retention`}>
